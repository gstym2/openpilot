...
def get_dynamic_follow(v_ego, personality=log.LongitudinalPersonality.standard):
  ...
  if personality==log.LongitudinalPersonality.relaxed:
    x_vel =  [0.,  6,   10., 10.01, 19.4, 27.7]
    y_dist = [1.7, 1.7, 1.9,  2.0,  2.1,  2.2]
  elif personality==log.LongitudinalPersonality.standard:
    x_vel =  [0.,  6,   10., 10.01, 19.4, 27.7]
    y_dist = [1.3, 1.4, 1.4,  1.45, 1.65, 1.95]
  elif personality==log.LongitudinalPersonality.aggressive:
    x_vel =  [0.,  6,   10., 19.4, 27.7]
    y_dist = [1.05, 1.15, 1.2, 1.3, 1.7]
  else:
...
def get_stopped_equivalence_factor(v_lead, v_ego):
  ...
  if np.all(delta_speed > 0.3):
    v_diff_offset = delta_speed * 1.75
...
class LongitudinalMpc:
  ...
  def set_weights(self, prev_accel_constraint=True, personality=log.LongitudinalPersonality.standard, v_lead0=0, v_lead1=0):
    ...
    if (v_lead0 - v_ego >= 0) and (v_lead1 - v_ego >= 0):
      j_ego_v_ego = np.interp(v_ego, v_ego_bps, [.75, 1.])
      a_change_v_ego = np.interp(v_ego, v_ego_bps, [.75, 1.])
    if self.mode == 'acc':
